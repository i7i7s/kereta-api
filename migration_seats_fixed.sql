...
-- FritzLine Railway App (FIXED VERSION)
-- ============================================

-- STEP 1: Update tabel seats dengan kolom booking status
-- Hapus dulu jika ada
...
-- STEP 3: Stored Procedure untuk auto-release kursi setelah timeout
DELIMITER $$
...
-- STEP 4: View untuk monitoring kursi available per kereta
...
-- STEP 5: View untuk booking history yang lebih detail
...
-- STEP 6: Trigger untuk sync seats.is_booked saat booking dikonfirmasi
DELIMITER $$
...
-- STEP 7: Event Scheduler untuk auto-release (jalankan setiap 5 menit)
-- Aktifkan event scheduler
SET GLOBAL event_scheduler = ON;

DROP EVENT IF EXISTS auto_release_seats;

CREATE EVENT auto_release_seats
ON SCHEDULE EVERY 5 MINUTE
DO CALL release_expired_seats();

-- Query test untuk verifikasi (uncomment untuk testing)
